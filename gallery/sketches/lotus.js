let layers = 29;
let leaf = 7;

function setup() {
  createCanvas(500, 500);
  strokeWeight(2);
  colorMode(HSB, 100);
  background(0);
}

function draw() {
  background(0, 4.7);
  stroke(0, (1+sin((millis()/10000)%10000)*PI)*50);

  for (let i = 0; i < layers; i++) {
    let timer = sin((millis() / 15000) * PI);
    let rotation = (PI / leaf) * timer * i;

    // size
    let size_ = min(width / 2, height / 2);
    let perlin = .7*pow(noise(millis() / 600.0 + i * 0.2), 2.4);
    let radius = size_ * (1-(i / layers));
    radius *= 1.5;

    // color
    let epylepsy = noise(millis() / 5000.0) * 400;
    let perlin2 = noise(millis() / 888.0 + i * 0.2 + 10000) * epylepsy % 100;
    fill(
      perlin2,
      randomGaussian(60, 3) + noise(millis()/100.0)*20,
      randomGaussian(100, 1),
      60
    );

    for (let i = 0; i < leaf; i++) {
      let c = cos(TAU * i / leaf + rotation);
      let d = sin(TAU * i / leaf + rotation);

      ellipse(width / 2 + (c * radius) / 2,
              height / 2 + (d * radius) / 2,
              radius * perlin,
              radius * perlin
             );
    }
  }
}
